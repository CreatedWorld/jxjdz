...
public class RecorderSystem
{
	...
	public void Update()
    {
        if (recorderFlag)
        {
            ...
            audioLength = (float)recordingPos / (float)recordFrequency;
            //满一次指定采样数就发送一次
            while (sampleIndex + recorderSampleSize <= recordingPos)
            {
                if (recorderFlag)
                    OnResample();
                //#if DEBUG
    ...
    public void Recording()
    {
        ...
        //以最高采样度进行采样
        recordFrequency = minFreq == 0 && maxFreq == 0 ? Frequency : maxFreq;
        //获取每秒采样率
        recorderSampleSize = recordFrequency * SampleSize / Frequency;
        sampleBuffer = new float[recorderSampleSize];
        Microphone.End(null);
        clip = Microphone.Start(device, false, MaxAudioLength, recordFrequency);
        recorderFlag = true;
    }
    ...
    void OnResample()
    {
        if (clip)
        {
            ...
            sampleIndex += recorderSampleSize;
