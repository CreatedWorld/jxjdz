...
namespace LZR.Data.NetWork.Client
{
    ...
    internal sealed class NSocketModel
    {
        ...
        /// 一个消息字节
        /// </summary>
        internal const long ConstLength = 4L;
        ...
        /// 构造函数
        /// </summary>
        internal NSocketModel(NSocket nSocket)
        {
        ...
        private Queue<byte[]> DecoderPack()
        {
            ...
            if (packLength == 0)
            {
                packLength = ReadInt(br.ReadBytes(4)) + 16;
            }
        ...
        /// <param name="channel"></param>
        /// <returns>Bye[]</returns>
        private byte[] Encoder(int channel, int type, byte[] buff)
        {
            try
            {
                ...
                bw.Write(WriterInt(NSocket.UserId));
                bw.Write(WriterInt(NSocket.RoomId));
                bw.Write(buff);
                buff = ms.ToArray();
                bw.Close();
                ms.Dispose();
